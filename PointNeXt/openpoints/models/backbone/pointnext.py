...
class SetAbstraction(nn.Module):
    ...
    def forward(self, pf):
        p, f = pf
        """
        print(f"\n--- SetAbstraction Forward Debug ---")
        print(f"Initial p.shape (points): {p.shape}")
        print(f"Initial f.shape (features): {f.shape}")
        print(f"Is head layer: {self.is_head}")
        print(f"Is all_aggr: {self.all_aggr}")
        print(f"Use residual (self.use_res): {self.use_res}") # Check this value
        """
        if self.is_head:
            f = self.convs(f)  # (n, c)
        else:
...
class PointNextEncoder(nn.Module):
    ...
    def forward_seg_feat(self, p0, f0=None):
        ...
        for i in range(0, len(self.encoder)):
            """
            print(f"\n--- Encoder Stage {i} Input ---")
            print(f"Input p shape to stage {i}: {p[-1].shape}")
            print(f"Input f shape to stage {i}: {f[-1].shape}")
            """
            _p, _f = self.encoder[i]([p[-1], f[-1]])
            """
            print(f"Output _p shape from stage {i}: {_p.shape}")
            print(f"Output _f shape from stage {i}: {_f.shape}")
            """
            p.append(_p)
            f.append(_f)
        return p, f
